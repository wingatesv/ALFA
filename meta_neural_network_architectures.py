...
class ResNet34(nn.Module):
    ...
    def build_network(self):
        ...
        for stage in range(len(num_chn)):
            for block in range(num_blocks[stage]):
                stride = 1
                if block == 0 and stage != 0:
                    stride = 2  # downsample at the beginning of each stage (except the first)
                self.layer_dict[f'layer{stage}_block{block}'] = MetaMaxResLayerReLU(
                    input_shape=out.shape,
                    num_filters=num_chn[stage],
                    kernel_size=3,
                    stride=stride,
                    padding=1,
                    use_bias=False,
                ...
                out = self.layer_dict[f'layer{stage}_block{block}'](out, training=True, num_step=0)
        ...
        out = self.layer_dict['linear'](out)
        print("ResNet34 build", out.shape)

    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False):
        """
        Forward propagates through the network. If any params are passed then they are used instead of stored params.
        :param x: Input image batch.
        :param num_step: The current inner loop step number
        :param params: If params are None then internal parameters are used. If params are a dictionary with keys the
         same as the layer names then they will be used instead.
        ...
        for stage in range(len(num_chn)):
            for block in range(num_blocks[stage]):
                out = self.layer_dict[f'layer{stage}_block{block}'](
                    out,
                    params=param_dict.get(f'layer{stage}_block{block}', None),
                    training=training,
                    backup_running_statistics=backup_running_statistics,
                    num_step=num_step
                )
        ...
        out = F.adaptive_avg_pool2d(out, (1, 1))
        out = out.view(out.size(0), -1)
        out = self.layer_dict['linear'](out, param_dict.get('linear', None))
    ...
    def restore_backup_stats(self):
        for stage in range(len(num_chn)):
            for block in range(num_blocks[stage]):
                self.layer_dict[f'layer{stage}_block{block}'].restore_backup_stats()
